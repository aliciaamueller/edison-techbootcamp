...
export default function AlarmRingingScreen({ navigation, route }) {
  ...
  const getMusicGenreUrl = () => {
    const genreUrls = {
      'energetic': 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3', // Upbeat electronic
      'calm': 'https://www.bensound.com/bensound-music/bensound-relaxing.mp3', // Peaceful ambient
      'rock': 'https://www.bensound.com/bensound-music/bensound-actionable.mp3', // Rock guitar
      'electronic': 'https://www.bensound.com/bensound-music/bensound-dubstep.mp3', // Electronic beats
    };
  ...
  const playAlarmSound = async () => {
    ...
    try {
      ...
      // Play QUIET background music that matches the genre
      const { sound } = await Audio.Sound.createAsync(
        { uri: getMusicGenreUrl() },
        { 
          shouldPlay: true,
          isLooping: true,
          volume: personality.soundVolume, // 8-20% volume based on personality
        }
      );
      
      soundRef.current = sound;
      console.log(`Playing ${musicGenre} music at ${personality.soundVolume * 100}% volume`);
    } catch (error) {
